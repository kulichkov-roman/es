...
if (0 < $arResult['SECTIONS_COUNT'])
{
    ...
    // получить картинки детальные
    $arIds = array();
    foreach($arResult["SECTIONS"] as &$arItem)
    {
        if($arItem["DETAIL_PICTURE"] <> "")
        {
            $arIds[] = $arItem["DETAIL_PICTURE"];
        }
    }
    unset($arItem);
    ...
    if(sizeof($arIds) > 0)
    {
        ...
        elseif(sizeof($arIds) == 1)
        {
            $strIds = current($arIds);
        }
        ...
        while($arItem = $rsFile->GetNext())
        {
            ...
            $urlDetailPicture = itc\Resizer::get($arItem["ID"], 'crop', 299, 200, $extension);
        ...
        foreach($arResult["SECTIONS"] as &$arItem)
        {
            if($arItem["DETAIL_PICTURE"] <> "")
            {
                $arItem["DETAIL_PICTURE"] = array(
                    "SRC" => $arDetailPicture[$arItem["DETAIL_PICTURE"]]["SRC"],
                );
            }
        }
        unset($arItem);
    ...
    // добавить заглушки
    $urlNoDetailPicture = itc\Resizer::get(NO_PHOTO_ID, 'crop', 299, 200, NO_PHOTO_EXTENSION);
