...
if (!empty($arResult['ITEMS']))
{
    ...
    {
        $arIds[] = $arItem["DETAIL_PICTURE"]["ID"];
    }
    unset($arItem);
    ...
    if(sizeof($arIds) > 0)
    {
        ...
        while($arItem = $rsFile->GetNext())
        {
            ...
            $urlPreviewPicture = itc\Resizer::get($arItem["ID"], 'width', 1920, null, $extension);
        ...
        foreach($arResult["ITEMS"] as &$arItem)
        {
            $arItem["DETAIL_PICTURE"]["SRC"] = $arPreviewPicture[$arItem["DETAIL_PICTURE"]["ID"]]["SRC"];
        }
        unset($arItem);
    }
}
