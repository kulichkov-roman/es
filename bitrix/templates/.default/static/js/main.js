	...
	$.fn.foxyLink = function(options) {
		...
		$wrapper.each(function() {
			...
			$wrapItem.on('touchstart.foxyLink', config.item, function(e) {
				...
				$('.menu__listItemLink').each(function() {$(this).removeClass('hover')});
		...
		$(document)
			...
			.on('touchstart.foxyLink', function(e) {
				...
				if (!$target.parents('.menu__listItem').find('.menu__listItemLink').length) {
					$('.menu__listItemLink').removeClass('hover');
				}
			});
...
$(function() {
    ...
    if ($objectMap.length) {
       ...
       function init() {
            ...
            objectsMap.behaviors.disable('multiTouch');

            objectsMap.controls
                .add('zoomControl', {left: 5, top: 90})
                .add('typeSelector')
                .add('mapTools', {left: 35, top: 90});
            ...
            $.each(markersData, function(index, value) {
                ...
                if (!value.coords.lat || !value.coords.lng) {
                    return;
                }

                var geo_objects = new ymaps.GeoObject({
                    geometry: {
                        type: 'Point',
                    ...
                    properties: {
                        hasBalloon: false,
                        iconContent:'<a href="' + value.url + '" class="objectBaloon"><div class="objectBaloon__header">' + value.header + '</div><img src="' + value.img + '" alt="" class="objectBaloon__image"/></a>',
...
$(function() {
    ...
    if ($('.services__listItemContentExpanderOpenerLink').length) {
        $('.services__listItemContentHeaderLink').on('click', function() {
            $(this).parents('.services__listItem').find('.services__listItemContentExpanderOpenerLink').trigger('click');
        });
    }
