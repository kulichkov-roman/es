...
iblock_submit => iblock_submit_dev
captcha_word  => captcha_word_dev
captcha_sid   => captcha_sid_dev
strIMessage   => strIMessage_dev
...
if (CModule::IncludeModule("iblock"))
{
	...
	if ($bAllowAccess)
	{
        ...
        if (check_bitrix_sessid() && (!empty($_REQUEST["iblock_submit_dev"]) || !empty($_REQUEST["iblock_apply"])) && empty($_REQUEST["NAME_USER_FALSE"]))
		{
			...
			{
				if (!$APPLICATION->CaptchaCheckCode($_REQUEST["captcha_word_dev"], $_REQUEST["captcha_sid_dev"]))
				{
			...
			if (count($arResult["ERRORS"]) == 0)
			{
				if (!empty($_REQUEST["iblock_submit_dev"]))
				{
				...
				$sRedirectUrl .= (strpos($sRedirectUrl, "?") === false ? "?" : "&")."strIMessage_dev=";
		...
		if ($bShowForm)
		{
			...
			$arResult["MESSAGE"] = htmlspecialcharsex($_REQUEST["strIMessage_dev"]);
