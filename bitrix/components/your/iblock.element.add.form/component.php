...
{
	// process POST data
	if (check_bitrix_sessid() && (!empty($_REQUEST["{$arParams['PREFIX_FORM']}_iblock_submit"]) || !empty($_REQUEST["{$arParams['PREFIX_FORM']}_iblock_apply"])) && empty($_REQUEST["NAME_USER_FALSE"]))
	{
		...
		{
			if (!$APPLICATION->CaptchaCheckCode($_REQUEST["{$arParams['PREFIX_FORM']}_captcha_word"], $_REQUEST["{$arParams['PREFIX_FORM']}_captcha_sid"]))
			{
		...
		if (empty($arResult["ERRORS"]))
		{
			...
			else
			{
				...
				if (!empty($_REQUEST["{$arParams['PREFIX_FORM']}_iblock_apply"]) && strlen($SEF_URL) > 0)
				{
		...
		if (empty($arResult["ERRORS"]))
		{
			if (!empty($_REQUEST["{$arParams['PREFIX_FORM']}_iblock_submit"]))
			{
			...
			$sRedirectUrl .= (strpos($sRedirectUrl, "?") === false ? "?" : "&")."{$arParams['PREFIX_FORM']}_strIMessage=";
	...
	if ($bShowForm)
	{
		...
		$arResult["MESSAGE"] = htmlspecialcharsex($_REQUEST["{$arParams['PREFIX_FORM']}_strIMessage"]);
