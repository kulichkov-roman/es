...
iblock_submit => iblock_submit_contact
captcha_word  => captcha_word_contact
captcha_sid   => captcha_sid_contact
strIMessage   => strIMessage_contact
...
if (CModule::IncludeModule("iblock"))
{
	...
	{
		// process POST data
		if (check_bitrix_sessid() && (!empty($_REQUEST["iblock_submit_contact"]) || !empty($_REQUEST["iblock_apply"])) && empty($_REQUEST["NAME_USER_FALSE"]))
		{
			...
			{
				if (!$APPLICATION->CaptchaCheckCode($_REQUEST["captcha_word_contact"], $_REQUEST["captcha_sid_contact"]))
				{
			...
			if (count($arResult["ERRORS"]) == 0)
			{
				if (!empty($_REQUEST["iblock_submit_contact"]))
				{
				...
				$sRedirectUrl .= (strpos($sRedirectUrl, "?") === false ? "?" : "&")."strIMessage_contact=";
		...
		if ($bShowForm)
		{
			...
			$arResult["MESSAGE"] = htmlspecialcharsex($_REQUEST["strIMessage_contact"]);
