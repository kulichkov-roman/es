...
AddEventHandler("iblock", "OnBeforeIBlockElementAdd", "UpdateSElementCode");
...
function UpdateElementCode(&$arFields)
{
    ...
    {
        // если нет $arFields["PROPERTY_VALUES"] свойства, то получить полное наименование
        if(!is_array($arFields["PROPERTY_VALUES"]))
        {
            $arSort = array();
            $arSelect = array("ID", "NAME", "PROPERTY_NAIMENOVANIE_DLYA_SAYTA_POLNOE");
            $arFilter = array("IBLOCK_ID" => CATALOG_IBLOCK_ID_KS, "ID" => $arFields["ID"]);
        ...
        {
            $strName = $arFields["PROPERTY_VALUES"][POLNOE_NAIMENOVANIE_PROP][POLNOE_NAIMENOVANIE_SUBPROP]["VALUE"];
        }
...
function UpdateSectionCode(&$arFields)
{
    if($arFields["IBLOCK_ID"] == CATALOG_IBLOCK_ID_KS)
    {
        // если нет $arFields["NAME"] свойства, то получить имя раздела
