...
AddEventHandler("iblock", "OnAfterIBlockElementAdd", Array("ITCElemAdd", "OnBeforeIBlockElementAddHandler"));
...
class ITCElemAdd { 
	...
	while ($arET = $rsET->Fetch()){
		...
		while($ar_props = $props->Fetch()){
			...
			case 'F':
				$ar_props['VALUE'] = 'http://'.$_SERVER['SERVER_NAME'].CFile::GetPath($ar_props['VALUE']);
				break;
